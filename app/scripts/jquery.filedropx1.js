...
(function($){
	...
	function getBuilder(filename, filedata, boundary, appendName) {
		var dashdash = '--',
			crlf = '\r\n',
			builder = '';
	...
	function upload() {
		...
		function send(e) {
			...
			if (typeof newName === "string") {
				builder = getBuilder(newName, e.target.result, boundary, true);
			} else {
				builder = getBuilder(file.name, e.target.result, boundary, true);
			}
			...
			//set format as query param
			//var filenameArray = file.name.split(".");
			
			var extension = getExtensionFromFilename(file.name);
			...
			//xhr.open("POST", opts.url+'?format='+fExt, true);
			xhr.open("POST", currentFolderRefUri+'?format='+documentumFormat, true);
